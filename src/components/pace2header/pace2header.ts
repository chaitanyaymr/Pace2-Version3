...
export class Pace2headerComponent {
  ...
  constructor(private appconst:PaceEnvironment, private db:DatabaseProvider,public camera:Camera,public actionSheetController: ActionSheetController){
...
setheader()
{
    this.db.getAllUsers().then(data=>{
      ...
      if(data[0].Emplogo!="" && data[0].Emplogo !=null)
      {empimg=this.appconst.Paceimg+"profile/"+data[0].Emplogo;}
      this.emplogo=empimg;
