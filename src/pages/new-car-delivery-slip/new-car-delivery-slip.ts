...
import { IonicPage, NavController, ToastOptions, ToastController, Platform } from 'ionic-angular';
...
@IonicPage({
  name:'page-newcarsold'
})
@Component({
  selector: 'page-new-car-delivery-slip',
  templateUrl: 'new-car-delivery-slip.html',
})
export class NewCarDeliverySlipPage {
...
;  constructor(public navCtrl: NavController,public scanner:BarcodeScanner,public OdsSvc:OdsServiceProvider,
    ...
    public platform:Platform) {
      ...
      this.db.getAllUsers().then(emdata=>{
        console.log("Newcar%Empdata%",emdata[0]);
    ...
    this.options={
     ...
     this.scanner.scan(this.options).then((data)=>{
      ...
      else if(data.text.length==18)
      {
        ...
        this.NewDeliveryData.StockNumber=this.NewDeliveryData.VIN.substring(18-this.stockcount_db); 
      }
      else{ 
      this.VINscannedData=data;
        this.NewDeliveryData.VIN=this.VINscannedData.text;    
        this.NewDeliveryData.StockNumber=this.NewDeliveryData.VIN.substring(17-this.stockcount_db);        
      }
  ...
  ResetScanTyp(){
    ...
    if(this.NewDeliveryData.VIN.length>=17)
      this.NewDeliveryData.StockNumber=this.NewDeliveryData.VIN.substring(this.NewDeliveryData.VIN.length-this.stockcount_db);        
    else
    this.NewDeliveryData.StockNumber="";
    }
  ...
  if(this.NewDeliveryData.StockNumber!="")
   {
     if(this.NewDeliveryData.StockNumber.length<this.stockcount_db)
   ...
   this.requestddl=this.requestddl == '0' ?this.empid :this.requestddl;
    ...
    wodetails=`<Info>
              <mwoid>0</mwoid> 
              <vin>`+this.NewDeliveryData.VIN.toUpperCase()+`</vin> 
              <stock>`+this.NewDeliveryData.StockNumber.toUpperCase()+`</stock> 
              <poid></poid> 
              <roid></roid> 
    ...
    let notes:any="";
    notes=`<Info><ssiid>`+this.ssiid+`</ssiid><notes>`+this.NewDeliveryData.Notes+`</notes><NType>W</NType></Info>`;
   let service:any="";
